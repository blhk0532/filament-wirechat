...
namespace AdultDate\FilamentWirechat\Livewire\Chats;
...
class Chats extends Component
{
    ...
    public function getListeners()
    {
        ...
        if ($this->panel() == null) {
            ...
            $listeners["echo-private:{$channelName},.AdultDate\\FilamentWirechat\\Events\\NotifyParticipant"] = 'refreshComponent';
        }
    ...
    public function mount()
    {
        ...
        // Extract ID from conversation parameter (could be object from route model binding or ID)
        $conversation = request()->conversation;
        $this->selectedConversationId = is_object($conversation) && method_exists($conversation, 'getKey')
            ? $conversation->getKey()
            : (is_object($conversation) && isset($conversation->id)
                ? $conversation->id
                : $conversation);
        $this->conversations = collect();
    ...
    protected function initialize()
    {
        ...
        // Heading defaults to empty string - leave as-is if not explicitly set
        // The heading can be set explicitly via the component properties

        //
        // BOOLEAN FLAGS
        //
        // Their default is null, so:
        //   • null → never set → use defaults
        //   • true/false → explicit override
        if ($this->createChatAction === null) {
            $this->createChatAction = true; // Default to enabled
        }

        if ($this->chatsSearch === null) {
            $this->chatsSearch = true; // Default to enabled
        }

        if ($this->redirectToHomeAction === null) {
            $this->redirectToHomeAction = $this->widget ? false : true; // Default based on context
        }
    }
    ...
    public function render()
    {
        ...
        return view('filament-wirechat::livewire.chats.chats');
    }
}
